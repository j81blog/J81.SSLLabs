...
#> 
    [OutputType('Qualys.SSLLabs.APIInfo.Object')]
    [CmdletBinding()]
    param(
        [Alias("Accept")]    
        [Switch]$AcceptTaC
    ...
    Process {
        try {
            $result = Invoke-RestMethod @RestParams -Verbose:$false
            <#
            ...
            #>
            $result.currentAssessments = try { [int32]::Parse($result.currentAssessments) } catch { 0 }
            $result.maxAssessments = try { [int32]::Parse($result.maxAssessments) } catch { 0 }
            $result.newAssessmentCoolOff = try { [int32]::Parse($result.newAssessmentCoolOff) } catch { 0 }

            if ($result.currentAssessments -ge $result.maxAssessments) {
                $result | Add-Member -MemberType NoteProperty -Name IsReady -Value $false
            } else {
                $result | Add-Member -MemberType NoteProperty -Name IsReady -Value $true
            }
            $result.messages | ForEach-Object {
                if ([String]$_ -like "?Private*") {
                    Write-Warning $_
                } else {
                    if (-Not $AcceptTaC) {
        ...
        try { $result.PSObject.TypeNames.Insert(0, "Qualys.SSLLabs.APIInfo.Object") } catch { }
        Write-Output $result
    }
    End {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCDcmOJ5FT6a32d
# oOZYd5kCm0/+uVRv+hNG3YcQwU78fKCCEHUwggTzMIID26ADAgECAhAsJ03zZBC0
...
# IgQgzNSXf0cAhH0aWonmrPFYNLAsy8a11kHbc0MIeKZ3EPcwDQYJKoZIhvcNAQEB
# BQAEggEARL/gkK45J+mFxE19ZuTTXeNW2D6RWUlPpZ/xY8xcVgwubcBbcEuCPO6S
# LgME+8CA2vLhAkdWX7KmqCBmFb7a4yDh4FhwJkjwOQ4XvbgmRvb6vQ1mAEiG92tE
# TMT/H77tzx9I0RiSZwPYMp14rbRSIFtTKECPFxwlSKK+atTewOetjZ4/ekbM22PY
# Yio76Aka8+9Rc2lPl1z7wVB5WKhdLknfTp0Q7eTK4GN8WLCz2bxS28jtRmDfztcD
# uGg7OCGRWpX5gvzoaXq5NfDdmI4KXPPddk4tbRdlygPv2D+RdrRhmAS7sHjLx+ML
# asqVx4+1j9+pe3yF1YzXvLwpM0sZRA==
